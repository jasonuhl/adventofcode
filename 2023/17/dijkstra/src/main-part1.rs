...
fn main() {
    ...
    let mut visited: Vec<Vec<Vec<Vec<Option<State>>>>> = vec![vec![vec![vec![None; 4]; 4]; a[0].len()]; a.len()];
    ...
    loop {
        if current.block.y == a[0].len() - 1 && current.block.x == a.len() - 1 {
        ...
        if current.block.y + 1 < a[0].len() {
            let candidate = Block{y: current.block.y + 1, x: current.block.x};
            if candidate != current.prior {
                neighbors.push(candidate);
            }
        }
        if current.block.x + 1 < a.len() {
            let candidate = Block{y: current.block.y, x: current.block.x + 1};
            if candidate != current.prior {
                neighbors.push(candidate);
            }
        }
        if current.block.y > 0 {
            let candidate = Block{y: current.block.y - 1, x: current.block.x};
            if candidate != current.prior {
                neighbors.push(candidate);
            }
        }
        if current.block.x > 0 {
            let candidate = Block{y: current.block.y, x: current.block.x - 1};
            if candidate != current.prior {
                neighbors.push(candidate);
            }
        ...
        for neigh in neighbors {
            ...
            let cum_cost = current_cum_cost + cell_cost;
            let mut straight_steps = 1;
            ...
            if straight_steps > 3 {
                //println!("Terminating path from {:?} to {:?}", current, neigh);
            } else {
